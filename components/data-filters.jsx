...
export const DataFilters = ({ onSelect, selectedFilters, onClear }) => {
    ...
    return (
        <div className="rounded-sm shadow-sm bg-slate-100 px-2 py-3">
            <div id="filter-header" className="flex flex-col gap-y-4">
            ...
            </div>
            {
                filters.map((filter) => {
                    return <div key={filter.title} className=" ">
                        <h2 className="py-4 text-[14px] uppercase font-medium">{filter.title}</h2>
                        <div className="flex flex-col gap-y-2">{filter.filters.map(item =>
